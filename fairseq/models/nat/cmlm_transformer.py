...
class CMLMNATransformerModel(NATransformerModel):
    ...
    def forward(
        ...
        '''
        # encoding
        encoder_out = self.encoder(src_tokens, src_lengths=src_lengths, **kwargs)  # encoder_padding_mask [B, T]
        '''
        encoder_out: [T, B, 512]
        encoder_padding_mask: [B, T]
        encoder_embedding: [B, T, 512]
        encoder_states: 0
        fc_results: 0
        src_tokens: 0
        src_lengths: [B, T]
        ...
        # decoding
        word_ins_out = self.decoder(        # prev_output_tokens -> output projection -> [B, T, 10152]
            normalize=False,
    ...
    def forward_decoder(self, decoder_out, encoder_out, decoding_format=None, **kwargs): # Only Use During Inference
        ...
        # _tokens: [  4,   8, 900,  18,  31, 360,   5,   4, 360, 360]
        # output_tokens: [0, 3, 3, 3, 3, 3, 3, 2, 1, 1]
        #  output_masks: [False, True, True, True, True, ... ]
        ...
        if (step + 1) < max_step:
            skeptical_mask = _skeptical_unmasking(  # p% 加入mask
