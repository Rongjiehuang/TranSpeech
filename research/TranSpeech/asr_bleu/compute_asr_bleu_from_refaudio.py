...
def compose_eval_data(
    ...
    reference_sentences = [f'{references_path}/{Path(file).stem}.wav' for file in predicted_audio_fp_list]
...
def run_asr_bleu(args):
    ...
    asr_config = retrieve_asr_config(
        args.lang, args.asr_version, json_path="/apdcephfs/private_nlphuang/Experiments/new_fairseq/fairseq_ust/examples/speech_to_speech/asr_bleu/asr_model_cfgs_custom.json" # "./asr_model_cfgs.json"
    )
    ...
    for _, eval_pair in tqdm(
        ...
        transcription = asr_model.transcribe_audiofile(eval_pair.reference)
        reference_transcripts.append(transcription.lower())
    ...
    bleu_score = sacrebleu.corpus_bleu(prediction_transcripts, [reference_transcripts])
...
def main():
    ...
    print(f"{args.audio_dirpath} | {args.reference_path}: {bleu_score}")

    result_filename = f"{args.lang}_bleu.txt"
