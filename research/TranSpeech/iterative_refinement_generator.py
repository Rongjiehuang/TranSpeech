...
# Note: this model is designed for non-autoregressive speech_to_speech translation
class IterativeRefinementGenerator(object):
    ...
    def generate(self, models, sample, prefix_tokens=None, constraints=None):
        ...
        bsz, src_len, src_dim = src_tokens.size()

        # initialize
        encoder_out = model.forward_encoder([src_tokens, src_lengths])
        prev_decoder_out = model.initialize_output_tokens(encoder_out, src_lengths)  # Note: here change [B, T, 80] [B]
        # [0 (start), 3 (mask), 3, 3, ..., 2 (stop), ..., 1 (pad)]
        ...
        if self.beam_size > 1:
            ...
            prev_decoder_out = model.regenerate_length_beam(  # Note: here change
    ...
    def rerank(self, reranker, finalized, encoder_input, beam_size):
        ...
        length_beam_order = (
            utils.new_arange(
                final_output_tokens, beam_size, reranker_encoder_out['encoder_out'][0].size(1) # reranker_encoder_out.encoder_out.size(1)
            )
            .t()
            .reshape(-1)
        )
