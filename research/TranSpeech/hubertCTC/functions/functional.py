...
def get_median(wav: torch.Tensor, sr: int) -> torch.Tensor:

    wav = peq(wav, sr)
    wav_numpy = wav.numpy()

    sound = wav_to_Sound(wav_numpy, sampling_frequency=sr)
    _, pitch_median = get_pitch_median(sound, None)

    return pitch_median

def manipulate_median(wav: torch.Tensor, sr: int, new_pitch_median) -> torch.Tensor:
    ...
    new_sound = change_gender(
        sound, pitch,
        1.0, new_pitch_median)

    wav = torch.from_numpy(new_sound.values).float().squeeze(0)
    return wav

def norm_energy(wav):
    wav = (wav.exp() ** 2).sum(-1).sqrt()
