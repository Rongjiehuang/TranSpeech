...
class InterpLnr(nn.Module):
    ...
    def forward(self, x, len_seq):  
        ...
        indices = torch.arange(self.max_len_seg*2, device=device)\
                  .unsqueeze(0).expand(batch_size*self.max_num_seg, -1)  # [batch_size*self.max_num_seg, self.max_len_seg*2]
        # scales of each sub segment
        scales = torch.rand(batch_size*self.max_num_seg, # [batch_size*self.max_num_seg]
                            device=device) + 0.5
        ...
        len_seg = torch.randint(low=self.min_len_seg,       # [batch_size*self.max_num_seg]
