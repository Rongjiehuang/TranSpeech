...
class InferenceProcessor:
    ...
    def process_sentence(
        ...
        if self.cfg.decoding.results_path is not None:
            # Here For S2S translation
            # sid = self.task.dataset(self.cfg.dataset.gen_subset).audio_names[sid]
            # print(f"({speaker}-{sid}) {hyp_pieces}", file=self.hypo_units_file)
            # print(f"({speaker}-{sid}) {hyp_words}", file=self.hypo_words_file)
            # print(f"({speaker}-{sid}) {tgt_pieces}", file=self.ref_units_file)
            # print(f"({speaker}-{sid}) {tgt_words}", file=self.ref_words_file)
        ...
        hyp_words, tgt_words = hyp_words.split(), tgt_words.split()  # Here For S2S translation
